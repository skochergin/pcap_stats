...
private:
    ...
    void pcap_loop(const pcap_pkthdr* pkth, const uint8_t* data)
    {
        ...
        if(l3_protocol == "IPv4")
        {
            ...
            if(l4_protocol == "TCP")
            {
                ...
                uint16_t computed_csum = compute_tcp_checksum(ip_header, reinterpret_cast<const uint16_t*>(tcp_start));
                std::cout << "TCP checksum: " << ntohs(tcp_header->csum) << "/" << computed_csum << std::endl;
            }
        }
    ...
    bool is_checksum_correct(const uint16_t* data, size_t size)
    {
        ...
        for(size_t i=0; i<size; i++)
            result += ntohs(data[i]);
        return static_cast<uint16_t>(result+2) == 0xFFFF;
    }
